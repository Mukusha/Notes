...
public class requestsSQLite {
    ...
    protected static ObservableList<TabularNoteView> FillingTV() throws  SQLException
    {

        ObservableList<TabularNoteView> listNodes = FXCollections.observableArrayList();
        resSet = statmt.executeQuery("SELECT dateModification,textSubject,isImportant  FROM notes");
        while(resSet.next())
        {
            listNodes.add(new TabularNoteView(new Note(
